  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center px-5 py-10 bg-gray-50">
      <form className="w-full max-w-md bg-white p-6 rounded shadow space-y-4" onSubmit={handleSubmit}>
        ...
        {!showReset && (
          <button  type="button" onClick={handleForgotPassword}  className="text-blue-600 text-sm font-medium hover:underline">
            Forgot Password?
          </button>
        )
      }
        ...
        {showReset && (
          <>
            ...
            <button type="button" onClick={handleResetPassword} className="bg-green-600 text-white w-full py-3 rounded" >
              Update Password
            </button>
          </>
        )}
